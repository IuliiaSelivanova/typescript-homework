...
const arrayWithNumbers = (...args) => {
    return args.filter((item) => typeof item === "number");
};
console.log(arrayWithNumbers(1, "res", 45, "vf", 24, 63, "cc", "cd"));
    ...
    if (typeof num1 === "number" &&
        ...
        return num1.slice(1).reduce((acc, val, index) => {
            if (!["+", "-", "*", "/"].includes(num2[index])) {
                throw new Error(`Некорректный оператор: ${num2[index]}`);
            }
            switch (num2[index]) {
                case "+":
                    return acc + val;
...
function applyOperation(array, operate) {
